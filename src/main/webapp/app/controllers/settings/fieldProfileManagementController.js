vireo.controller("FieldProfileManagementController", function ($q, $controller, $scope, $filter, DragAndDropListenerFactory, DocumentTypeRepo, FieldProfileRepo, OrganizationRepo, ControlledVocabularyRepo, FieldPredicateRepo, InputTypeRepo, WorkflowStepRepo, ManagedConfigurationRepo) {
    ...
    $scope.selectedOrganization = function () {
        console.log("DEBUG: FPM, getting selected org");
    ...
    $scope.selectedOrganizationId = function () {
        console.log("DEBUG: FPM, selectedOrganizationId");
    ...
    $scope.selectedOrganizationName = function () {
        console.log("DEBUG: FPM, selectedOrganizationName");
    ...
    $scope.isEditable = function (fieldProfile) {
        ...
        if (!editable && !!OrganizationRepo.selectedOrganizationId()) {
            editable = fieldProfile.originatingWorkflowStep == $scope.step.id && OrganizationRepo.selectedOrganization().originalWorkflowSteps.indexOf(fieldProfile.originatingWorkflowStep) > -1;
        }
        return editable;
    };
    ...
    $q.all([ManagedConfigurationRepo.ready(), ControlledVocabularyRepo.ready(), FieldPredicateRepo.ready(), InputTypeRepo.ready(), DocumentTypeRepo.ready(), OrganizationRepo.ready()]).then(function () {
        console.log("DEBUG: fp, is now ready."); // FIXME: this is called 4x, there are four instances!?
        ...
        FieldPredicateRepo.listen(function () {
          console.log("DEBUG: fp, is rebuilding list.");
            $scope.buildFilteredPredicateList();
        });

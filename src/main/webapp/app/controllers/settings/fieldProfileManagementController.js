vireo.controller("FieldProfileManagementController", function ($q, $controller, $scope, $filter, DragAndDropListenerFactory, DocumentTypeRepo, FieldProfileRepo, OrganizationRepo, ControlledVocabularyRepo, FieldPredicateRepo, InputTypeRepo, WorkflowStepRepo, ManagedConfigurationRepo) {
    ...
    $scope.isEditable = function (fieldProfile) {
        ...
        if (!editable) {
            editable = fieldProfile.originatingWorkflowStep == $scope.step.id && $scope.organizationRepo.findSelectedOrganization().originalWorkflowSteps.indexOf(fieldProfile.originatingWorkflowStep) > -1;
        }
        return editable;
    };
    ...
    $scope.setSelectedOrganization = function (organization) {
        var existingId = !!$scope.selectedOrganization && !!$scope.selectedOrganization.id ? $scope.selectedOrganization.id : undefined;
        ...
        } else if (!organization && !organization.id && existingId !== $scope.selectedOrganization.id) {
            $scope.buildFilteredPredicateList();
        }
    };

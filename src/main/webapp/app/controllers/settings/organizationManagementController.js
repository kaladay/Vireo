vireo.controller("OrganizationManagementController", function ($controller, $location, $q, $route, $scope, $timeout, AccordionService, AlertService, ApiResponseActions, Organization, OrganizationRepo, OrganizationCategoryRepo, WorkflowStepRepo) {
    ...
    $scope.showOrganizationManagement = function () {
        return !!OrganizationRepo.selectedOrganization() && !!OrganizationRepo.selectedOrganization().id && (OrganizationRepo.selectedOrganization().id !== 1 || (OrganizationRepo.selectedOrganization().id == 1 && $scope.isAdmin()));
    };
    ...
    $scope.resetManageOrganization = function () {
        ...
        if (!!OrganizationRepo.selectedOrganization() && !!OrganizationRepo.selectedOrganization().id) {
            ...
            console.log("DEBUG: selectedOrganization = ", OrganizationRepo.selectedOrganization());
    ...
    $scope.setSelectedOrganization = function (organization, specific) {
        ...
        if (!organization || !organization.id || !organization.complete && !organization.dirty()) {
            console.log("DEBUG: (1) set selected, reload shallow, organization = ", organization);
            OrganizationRepo.getSelectedOrganization('shallow').then(function (org) {
            ...
            console.log("DEBUG: (2) set selected, organization = ", organization);

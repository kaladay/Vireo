vireo.controller("NoteManagementController", function ($controller, $scope, DragAndDropListenerFactory, Note, NoteRepo, OrganizationRepo, WorkflowStepRepo) {
    ...
    $scope.isEditable = function(note) {
        ...
        if(!editable && !!OrganizationRepo.selectedOrganization()) {
            editable = note.originatingWorkflowStep == $scope.step.id && OrganizationRepo.selectedOrganization().originalWorkflowSteps.indexOf(note.originatingWorkflowStep) > -1;
        }
        return editable;
    };

vireo.controller('NewSubmissionController', function ($controller, $location, $q, $scope, OrganizationForStudentRepo, StudentSubmissionRepo, ManagedConfigurationRepo, SubmissionStates) {
    ...
    $scope.getSelectedOrganization = function () {
        ...
        if ($scope.ready) {
            var selectedId = OrganizationForStudentRepo.getSelectedId();
    ...
    $scope.setSelectedOrganization = function (organization) {
        OrganizationForStudentRepo.setSelectedOrganization(organization);
    ...
    $q.all([ManagedConfigurationRepo.ready(), StudentSubmissionRepo.ready()]).then(function () {
        OrganizationForStudentRepo.defer().then(function (orgs) {
            if (!!orgs && orgs.length > 0) {
                OrganizationForStudentRepo.setSelectedOrganization(orgs[0]);
